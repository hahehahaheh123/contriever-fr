...
def load_retriever(model_path, pooling="average", random_init=False):
  # load actual model given model_path (similar to load(), it's just that we wrap the Contriever class around it)
  ...
  if os.path.exists(path):
    ...
    # **if statement for xlmretriever/contriever choice
    model_class = Contriever
    retriever = model_class(cfg)
    pretrained_dict = pretrained_dict["model"]  # get nested dict
    ...
    if any("encoder_q." in key for key in pretrained_dict.keys()):
      ...
      # ^ for any key containing 'encoder_q.' in key name, remove it from name
