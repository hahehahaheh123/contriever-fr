# Exact same thing, except added comments.
import torch
import torch.distributed as dist


class Gather(torch.autograd.Function):
    @staticmethod  # used when fct is not dependant on class in any way
...
def gather(x: torch.tensor):
    ...
    return x_gather
# the action of gathering is non differentiable, hence we must define it as an autograd.Function object: this also
# means that we need to define its own forward and backward sequence. I think that gathering requires that we
# take into account the gradients of every gathered tensor, hence the summation (all_reduce())

# For each process, while we gather all tensors we also collect the processes' tensor and put it in output

@torch.no_grad()
def gather_nograd(x: torch.tensor):
    if not dist.is_initialized():
        return x
    x_gather = [torch.ones_like(x) for _ in range(dist.get_world_size())]  # dunno if zeros and ones matter
...
def varsize_gather_nograd(x: torch.Tensor):
    """gather tensors of different sizes along the first dimension
       1. Gather all sizes
       2. Gather max size (column length)
       3. Gather all tensors
       4. Pad them according to max size
    """
    if not dist.is_initialized():
        return x
