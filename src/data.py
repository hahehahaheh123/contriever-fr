...
def add_bos_eos(x, bos_token_id, eos_token_id):
  # "beginning token - x - ending token" sandwich, but only when specified
...
class MultiDataset(torch.utils.data.Dataset):
  ...
  def set_prob(self, coeff=0.0):
    # probs of being picked based on length of dataset, by default the probs are uniform (coeff=0.0)
    prob = np.array([float(len(dataset)) for _, dataset in self.dataset.items()])
    prob /= prob.sum()
...
def load_data(opt, tokenizer):
  ...
  dataset = MultiDataset(datasets)  # combine multiple dataset objects into one
