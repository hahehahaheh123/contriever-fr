...
class MoCo(nn.Module):
  ...
  @torch.no_grad()
  def _dequeue_and_enqueue(self, keys):  # they use a different way to do "last in, first out", still the same
    # gather keys before updating queue
    keys = dist_utils.gather_nograd(keys.contiguous())  # returns one concatenated tensor
  ...
  def _compute_logits(self, q, k):
    l_pos = torch.einsum("nc,nc->n", [q, k]).unsqueeze(-1)  # diagonal only mat mul (2048, 1)
    l_neg = torch.einsum("nc,ck->nk", [q, self.queue.clone().detach()])  # matrix multiplication (2048, 131072)
  ...
  def forward(self, q_tokens, q_mask, k_tokens, k_mask, stats_prefix="", iter_stats={}, **kwargs):
    ...
    # perform mat mul (dot prods) for logits
    logits = self._compute_logits(q, k) / self.temperature
    
    # labels: positive key indicators
    labels = torch.zeros(bsz, dtype=torch.long).cuda()  # bc positive is always first
    
    loss = torch.nn.functional.cross_entropy(logits, labels, label_smoothing=self.label_smoothing)
    
    self._dequeue_and_enqueue(k)  # moco operates with batch and queue seperately
    ...
    stdq = torch.std(q, dim=0).mean().item()  # standard dev of embeddings
